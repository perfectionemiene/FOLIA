 ...
 * Accepts File / Blob / ArrayBuffer. Blobs are stored natively by IndexedDB
 * (most efficient) and are reliably parsed by epub.js and pdf.js.
 */
async function saveBookFile(bookId, fileData) {
  try {
    ...
    throw error; // let callers know the save failed
  }
}
...
async function getBookFile(bookId) {
  try {
    await ensureLocalForage();
    return await localforage.getItem(`book_file_${bookId}`);
  } catch (error) {
